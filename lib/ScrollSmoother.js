    ...
    _autoDistance = (el, progress) => {
        ...
        if (change > 0) {
            ...
            extraChange =
                ratio === 0.5
                    ? b2.height * 2
                    : Math.min(b2.height, (-change * ratio) / (2 * ratio - 1)) *
                      2 *
                      (progress || 1)
    ...
    constructor(vars) {
        ...
        let {
            ...
            resizeObserver =
                ...
                new ResizeObserver(() => mainST.refresh()),
            ...
            _onFocusIn = (e) => {
                ...
                if (
                    ...
                    (onFocusIn && onFocusIn(this, e) === false)
                ) {
                    return
                }
                ScrollTrigger.isInViewport(e.target) ||
                    e.target === lastFocusElement ||
                    this.scrollTo(e.target, false, 'center center')
                ...
                effects.forEach((st) => {
                    ...
                    triggers.forEach((trig) => {
                        if (
                            ...
                            pins.forEach(
                                (p) =>
                                    (dif -= p.distance / st.ratio - p.distance)
                            )
                ...
                if (effects) {
                    ...
                    effects.forEach((st) => {
                        let start = st.start,
                                ...
                                ? Math.min(
                                      ScrollTrigger.maxScroll(st.scroller),
                                      st.end
                                  )
            ...
            restoreEffects = () =>
                effects && effects.forEach((st) => st.vars.onRefresh(st)),
            revertEffects = () => {
            ...
            effectValueGetter = (name, value, index, el) => {
                return () => {
                    let v =
                        typeof value === 'function' ? value(index, el) : value
                    v ||
                        v === 0 ||
                        (v =
                ...
                if (ratio !== 1 || autoSpeed || scrub) {
                    st = ScrollTrigger.create({
                        ...
                        onUpdate: (self) => {
                            let y =
                                    startY +
                                    change * (self.progress - progressOffset),
                                i = pins.length,
                                extraY = 0,
                                pin,
                                scrollY,
                                            ...
                                            if (scrub) {
                                                pin.trig.progress +=
                                                    pin.trig.direction < 0
                                                        ? 0.001
                                                        : -0.001 // just to make absolutely sure that it renders (if the progress didn't change, it'll skip)
        ...
        this.scrollTo = (target, smooth, position) => {
            let p = gsap.utils.clamp(
                ...
                isNaN(target) ? this.offset(target, position) : +target
            )
            !smooth
                ? scrollTop(p)
                : paused
        ...
        this.sections = (targets, config) => {
            ...
            let newSections = _toArray(targets).map((el) =>
        ...
        let existingScrollTriggers = ScrollTrigger.getAll().filter(
            (st) => st.scroller === _win || st.scroller === wrapper
        )
        ...
        this.kill = () => {
            ...
            defaults &&
                ...
                ScrollTrigger.defaults({ scroller: _win })
        ...
        if (normalizeScroll) {
            this.normalizer = ScrollTrigger.normalizeScroll(
                normalizeScroll === true
                    ? { debounce: true, content: !smoothDuration && content }
                    : normalizeScroll
            )
        }
        ...
        'overscrollBehavior' in _win.getComputedStyle(_body) &&
            gsap.set([_body, _docEl], { overscrollBehavior: 'none' })
        'scrollBehavior' in _win.getComputedStyle(_body) &&
            gsap.set([_body, _docEl], { scrollBehavior: 'auto' })
